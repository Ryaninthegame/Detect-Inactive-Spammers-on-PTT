...
class net(nn.Module):
    def __init__(self, withSuspectValue):
        super(net, self).__init__()
        self.relu = nn.LeakyReLU(0.1)
        if withSuspectValue:
            self.GraphConv1 = GraphConv(73, 32, weight=True, bias=True)
        else:
            self.GraphConv1 = GraphConv(72, 32, weight=True, bias=True)
...
if __name__ == "__main__":
    ...
    _withSuspectValue, _run, _cutting, _epoch = _args.withSuspectValue, _args.run, _args.cutting, _args.epoch
    ...
    for _i in range(_run):   
        ...
        _savePath = './GCN_' + str(_i) + '.pth'
        model = net(_withSuspectValue)
        model.cuda()
        ...
        _endTime = time.time()
        print(_i, "Time :", _endTime-_startTime, "(s)", "done")